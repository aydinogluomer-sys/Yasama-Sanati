...
export default function RotatedTypeMass({
  ...
  return (
    ...
    >
      ...
      >
        {words.map((w, i) => (
          <span key={i} className="inline-flex items-baseline">
            ...
            <span className="mx-[0.18em] text-[0.3em] text-copper-text">✦</span>
          </span>
        ))}
      </motion.div>
      ...
      {rotatedWord && (
        <span
          className="pointer-events-none absolute right-3 top-1/2 hidden -translate-y-1/2 select-none font-mono text-[0.78rem] uppercase tracking-[0.22em] text-copper-text [writing-mode:vertical-rl] md:block"
        >
          {rotatedWord}
        </span>
      )}
