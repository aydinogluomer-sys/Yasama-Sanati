...
const TherapyScene3D = dynamic(
  () => import("./TherapyScene3D"),
  {
    ssr: false,
    loading: () => (
      <div className="h-[520px] md:h-[600px] bg-[#1a2420] rounded border border-cream/10 animate-pulse flex items-center justify-center text-sm text-cream/85">
...
export default function TherapyScene3DWrapper() {
  ...
  if (prefersReducedMotion || isMobile === null || isMobile) {
    return (
      <div className="rounded border border-cream/10 bg-[#1a2420] p-6 md:p-8 text-cream/80">
        <div className="space-y-3">
          <p className="text-4xs uppercase tracking-[0.3em] text-cream/85">3D Terapi Sahnesi</p>
          <h3 className="text-24 md:text-30 font-light text-white">Bedenin enerji haritası</h3>
          <p className="max-w-2xl text-sm md:text-base leading-relaxed text-cream/85">
