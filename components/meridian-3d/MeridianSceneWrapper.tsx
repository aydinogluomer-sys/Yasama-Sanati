...
const MeridianScene = dynamic(
  () => import("./MeridianScene"),
  {
    ssr: false,
    loading: () => (
      <div className="h-[650px] md:h-[750px] bg-[#030806] rounded-2xl border border-white/[0.08] animate-pulse flex items-center justify-center text-sm text-sage/85">
      ...
      {useStaticFallback ? (
        ...
        <div className="flex h-full min-h-[560px] items-center justify-center bg-[#030806] px-6 text-center">
          <div className="max-w-md rounded-2xl border border-white/[0.08] bg-[#0b1411]/90 p-7 shadow-2xl">
            <span className="text-4xs font-medium uppercase tracking-[0.24em] text-copper-text">Erişilebilir görünüm</span>
            <h3 className="mt-3 text-xl font-medium text-white">Bedenin enerji haritası</h3>
            <p className="mt-3 text-sm leading-7 text-sage">{SCENE_DESCRIPTION}</p>
            <p className="mt-4 text-xs leading-6 text-sage/85">Akciğer, kalp, mide, böbrek, karaciğer ve kalın bağırsak meridyenleri bu deneyimde incelenebilir.</p>
          </div>
        </div>
